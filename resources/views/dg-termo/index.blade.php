...
<div id="no" class="section-heading">
	<h2> <b> "DATOS GENERALES TERMOFIJADO" <br> </b> <br> Control y Administracion de Datos </h2>
</div>
...
<div id="no" class="datoss" style="background-color: #e6e6e6; float:left; width: 63% ">
			<center>
			<h4>Listado de todos los Datos Generales Termofijado y registro de nuevas datos al sistema.</h4><br>
			
			<a href=" {{ route('dgtermofijados.create') }}" type="submit" class="btn btn-default" 
...
&nbsp;&nbsp;&nbsp;
<h2 id="no">Listado de Datos Generales Termofijado</h2>
...
<!-- E   N    D         B   U   S   C   A   D   O   R       -->
		...
		<table id="mytable" class="table table-hover table-striped">
			...
			<tbody>
				 @foreach($dgtermos as $ter)
				 <tr>
				 	<td> {{ $ter->Diseño_Termo }} </td>
				 	<td>{{ $ter->Proceso }}</td>
				 	<td style="font-size: 12px;"> {{ $ter->updated_at }} </td>
				 	<!--<td> {{ $ter->horaE_DGTE }} </td>-->
				 	<td> {{ $ter->Npieza_DGTE }} </td>
				 	<td> {{ $ter->color_DGTE }} </td>
				 	<td> {{ $ter->loteProd_DGTE }} </td>
				 	<td> {{ $ter->equipo_DGTE }} </td>
				 	<td> {{ $ter->batch_DGTE }} </td>
				 	<td> {{ $ter->loteUrdido_DGTE }} </td>
				 	<td> {{ $ter->folioBond_DGTE }} </td>
				 	<td> {{ $ter->codProdTermin_DGTE }} </td>
				 	<td> {{ $ter->nombreOpera_DGTE }} </td>
				 	<td> {{ $ter->comentarios_DGTE }} </td>
				 	
				 	
				 	
				 	<td id="no">
				 		<a type="submit" class="btn btn-succes" href=" {{ route('dgtermofijados.edit', $ter->id_DGTE) }} " style="background: #009900; 
											  border: 0;
											  width: 100px;
											  padding: 10px;
											  height: 40px;
				 	...
				 	<td id="no">
				 		<form action=" {{ route('dgtermofijados.destroy', $ter->id_DGTE) }} " method="POST" >
				 			{{ csrf_field() }}
