...
<div id="no" class="section-heading">
	<h2> <b> "DATOS GENERALES BACKING" <br> </b> <br> Control y Administracion de Datos </h2>
</div>
...
<div id="no" class="datoss" style="background-color: #e6e6e6; float:left; width: 63% ">
			<center>
			<h4>Listado de todos los Datos Generales Backing y registro de nuevas datos al sistema.</h4><br>
			
			<a href=" {{ route('dgbackings.create') }}" type="submit" class="btn btn-default" 
...
<h2 id="no">Listado de Datos Generales Backing</h2>
...
<!-- E   N    D         B   U   S   C   A   D   O   R       -->
		...
		<table id="mytable" class="table table-hover table-striped">
			...
			<tbody>
				 @foreach($dgbacki as $bac)
				 <tr>
				 	<td> {{ $bac->Diseño_Back }} </td>
				 	<td>{{ $bac->Proceso }}</td>
				 	<td style="font-size: 12px;"> {{ $bac->updated_at }} </td>
				 	<!--<td> {{ $bac->horaE_DGB }} </td>-->
				 	<td> {{ $bac->Npieza_DGB }} </td>
				 	<td> {{ $bac->color_DGB }} </td>
				 	<td> {{ $bac->loteProd_DGB }} </td>
				 	<td> {{ $bac->equipo_DGB }} </td>
				 	<td> {{ $bac->batch_DGB }} </td>
				 	<td> {{ $bac->loteUrdido_DGB }} </td>
				 	<td> {{ $bac->folioBond_DGB }} </td>
				 	<td> {{ $bac->codProdTermin_DGB }} </td>
				 	<td> {{ $bac->nombreOpera_DGB }} </td>
				 	<td> {{ $bac->comentarios_DGB }} </td>
				 	
				 	
				 	
				 	<td id="no">
				 		<a type="submit" class="btn btn-succes" href=" {{ route('dgbackings.edit', $bac->id_DGB) }} " style="background: #009900; 
											  border: 0;
											  width: 100px;
											  padding: 10px;
											  height: 40px;
				 	...
				 	<td id="no">
				 		<form action=" {{ route('dgbackings.destroy', $bac->id_DGB) }} " method="POST" >
				 			{{ csrf_field() }}
