...
<div class="section-heading">
	<h2> <b> "DATOS GENERALES REPELENTE" <br> </b> <br> Control y Administracion de Datos </h2>
</div>
...
<div class="datoss" style="background-color: #e6e6e6; float:left; width: 63% ">
			<center>
			<h4>Listado de todos los Datos Generales Repelente y registro de nuevas datos al sistema.</h4><br>
			
			<a href=" {{ route('dgrepelentes.create') }}" type="submit" class="btn btn-default" 
		...
		<table id="mytable" class="table table-hover table-striped">
			...
			<tbody>
				 ...
				 <tr>
				 	...
				 	<td> {{ $rep->color_DGR }} </td>
				 	<td> {{ $rep->loteProd_DGR }} </td>
				 	<td> {{ $rep->equipo_DGR }} </td>
				 	<td> {{ $rep->batch_DGR }} </td>
				 	<td> {{ $rep->loteUrdido_DGR }} </td>
				 	<td> {{ $rep->folioBond_DGR }} </td>
				 	<td> {{ $rep->codProdTermin_DGR }} </td>
				 	<td> {{ $rep->nombreOpera_DGR }} </td>
				 	<td> {{ $rep->comentarios_DGR }} </td>
				 	
				 	
				 	
				 	<td>
				 		<a type="submit" class="btn btn-succes" href=" {{ route('dgrepelentes.edit', $rep->id_DGR) }} " style="background: #009900; 
											  border: 0;
											  width: 100px;
											  padding: 10px;
											  height: 40px;
				 	...
				 	<td>
				 		<form action=" {{ route('dgrepelentes.destroy', $rep->id_DGR) }} " method="POST" >
				 			{{ csrf_field() }}
