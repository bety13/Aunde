...
<div id="no" class="section-heading">
	<h2> <b> "PROCESOS DE ÁREA LAVADO" <br> </b> <br> Control y Administracion de Datos </h2>
</div>
...
<div id="no" class="datoss" style="background-color: #e6e6e6; float:left; width: 63% ">
			<center>
			<h4>Listado de todos los Procesos Lavado y registro de nuevos Procesos al sistema.</h4><br>
...
<h2 id="no">Listado de Procesos Area Lavado</h2>
...
<!-- E   N    D         B   U   S   C   A   D   O   R       -->
		...
		<table id="mytable" class="table table-hover table-striped">
			...
			<tbody>
				 @foreach($procesolava as $prolav)
				 <tr>
				 	<td> {{ $prolav->id_Ramas }} </td>
				 	<td> {{ $prolav->pOrilla_PLav }} </td>
				 	<td> {{ $prolav->pCentro_PLav }} </td>
				 	<td> {{ $prolav->ppOrilla_PLav }} </td>
				 	<td> {{ $prolav->promedio_PLav }} </td>
				 	<td> {{ $prolav->anchoU_PLav }} </td>
				 	<td> {{ $prolav->apariencia_PLav }} </td>
				 	<td> {{ $prolav->disposicion_PLav }} </td>
				 	<td> {{ $prolav->fechaE_PLav }} </td>
				 	<td> {{ $prolav->horaE_PLav }} </td>
				 	<td> {{ $prolav->analista_PLav }} </td>
				 	<td> {{ $prolav->comentario_PLav }} </td>
				 	
				 	
				 	
				 	<td id="no">
				 		<a type="submit" class="btn btn-succes" href=" {{ route('procesolavados.edit', $prolav->id_PLav) }} " style="background: #009900; 
											  border: 0;
											  width: 100px;
											  padding: 10px;
											  height: 40px;
				 	...
				 	<td id="no">
				 		<form action=" {{ route('procesolavados.destroy', $prolav->id_PLav) }} " method="POST" >
				 			{{ csrf_field() }}
...
<input type="text" name="pOrilla_PLav" class="cantidad" placeholder="0"><br><br>

<label style="font-size: 16px; color: black">Peso Centro :</label>
<input type="text" name="pCentro_PLav" class="cantidad" placeholder="0"><br><br>

<label style="font-size: 16px; color: black">Peso Orilla :</label>&nbsp;&nbsp;
<input type="text" name="ppOrilla_PLav" class="cantidad" placeholder="0"><br>
<br/>
...
<input type="text" name="promedio_PLav" style="width: 83px;"><br><br>

<label style="font-size: 16px; color: black; ">Ancho Util :</label>&nbsp;&nbsp;&nbsp;
{!! Form::text('anchoU_PLav', null, ['class' => 'form-control'] ) !!}<br><br>

<label style="font-size: 16px; color: black; ">Apariencia :</label>&nbsp;&nbsp;&nbsp;
{!! Form::text('apariencia_PLav', null, ['class' => 'form-control'] ) !!}<br><br>
...
{!! Form::select('disposicion_PLav', ['' => 'Seleccionar', 'Liberado' => 'Liberado', 'Rechazado' => 'Rechazado', 'Segunda calidad' => 'Segunda Calidad'] , ['class' => 'form-control'] ) !!}<br><br>

<label style="font-size: 16px; color: black; ">Fecha de Entrega :</label>&nbsp;&nbsp;&nbsp;
{!! Form::text('fechaE_PLav', null, ['class' => 'form-control'] ) !!}<br><br>
	

<label style="font-size: 16px; color: black; ">Hora de Entrega :</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	{!! Form::text('horaE_PLav', null, ['class' => 'form-control'] ) !!}<br><br>
	

<label style="font-size: 16px; color: black; ">Analista :</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	{!! Form::text('analista_PLav', null, ['class' => 'form-control'] ) !!}<br><br>
	
<label style="font-size: 16px; color: black; ">Comentario :</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	{!! Form::text('comentario_PLav', null, ['class' => 'form-control'] ) !!}<br><br>
