...
<div id="no" class="section-heading">
	<h2>"Control y Administracion de Telas PUNZONADO"</h2>
</div>
...
<div id="no" class="datoss" style="background-color: #e6e6e6; float:left; width: 63% ">
			<center>
			<h4>Listado de todas las Telas de area Punzonado y registro de nuevas telas al sistema.</h4><br>
			
			<a href=" {{ route('telaspunzonados.create') }}" type="submit" class="btn btn-default" 
...
<h2 id="no">Listado de Telas Punzonado</h2>
		...
		<table id="mytable" class="table table-hover table-striped">
			...
			<tbody>
				 ...
				 <tr>
				 	...
				 	<td> {{ $p->Diseño_Punzo }} </td>
				 	<td> {{ $p->color_Punzo }} </td>
				 	<td> {{ $p->OEM_Punzo }} </td>
				 	<td> {{ $p->MP_peso_Punzo }} </td>
				 	<td> {{ $p->MP_anchoU_Punzo }} </td>
				 	<td> {{ $p->MP_apari_Punzo }} </td>
				 	<td> {{ $p->UM_peso_Punzo }} </td>
				 	<td> {{ $p->UM_anchoU_Punzo }} </td>
				 	<td> {{ $p->UM_apari_Punzo }} </td>
				 	<td> {{ $p->LSE_peso_Punzo }} </td>
				 	<td> {{ $p->LSE_anchoU_Punzo }} </td>
				 	<td> {{ $p->LSE_apari_Punzo }} </td>
				 	<td> {{ $p->LIE_peso_Punzo }} </td>
				 	<td> {{ $p->LIE_anchoU_Punzo }} </td>
				 	<td> {{ $p->LIE_apari_Punzo }} </td>
				 	
				 	<td id="no">
				 		<a type="submit" class="btn btn-succes" href=" {{ route('telaspunzonados.edit', $p->id_Punzo) }} " style="background: #009900; 
											  border: 0;
											  width: 100px;
											  padding: 10px;
											  height: 40px;
				 	...
				 	<td id="no">
				 		<form action=" {{ route('telaspunzonados.destroy', $p->id_Punzo) }} " method="POST" >
				 			{{ csrf_field() }}
