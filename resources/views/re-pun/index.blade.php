...
<div id="no" class="datoss" style="background-color: #e6e6e6; float:left; width: 63% ">
			...
			<a href=" {{ route('reprocesopunzonados.create') }}" type="submit" class="btn btn-default" 
...
<!-- E   N    D         B   U   S   C   A   D   O   R       -->
		...
		<table id="mytable" class="table table-hover table-striped">
			...
			<tbody>
				 @foreach($repunzo as $repun)
				 <tr>
				 	<td> {{ $repun->pOrilla_RePunzo }} </td>
				 	<td> {{ $repun->pCentro_RePunzo }} </td>
				 	<td> {{ $repun->ppOrilla_RePunzo }} </td>
				 	<td> {{ $repun->pPromedio_RePunzo }} </td>
				 	<td> {{ $repun->anchoU_RePunzo }} </td>
				 	<td> {{ $repun->apariencia_RePunzo }} </td>
				 	<td> {{ $repun->disposicion_RePunzo }} </td>
				 	<td> {{ $repun->fechaE_RePunzo }} </td>
				 	<td> {{ $repun->horaE_RePunzo }} </td>
				 	<td> {{ $repun->analista_RePunzo }} </td>
				 	<td> {{ $repun->comentario_RePunzo }} </td>
